...
**$87 per 1k requests** at 25-50k context to **$164** at 175-200k. The
post-compact base is **~0.35× the window**, and context grows only **~900
tokens per request** (median). So compaction is cheap and carrying context is
not — anything parked in context is paid for again on every later request.
That single fact drives all the rules below.
...
3. **`/clear` on task switch, `/compact` only mid-task.** Compaction is cheap
   (summary output ~1.4k) and the post-compact base is ~30% of the window.
   Carrying a stale 100k context is the expensive thing, not resetting it.
4. **Do not raise `autoCompactWindow` to "avoid compaction."** It is a false
   economy — cost per request scales with the window. Global is 110k on every
   box, which puts average live context at ~74k, the top of the cheap band.
5. **Pick the subagent model per stage.** Measured cost per agent: `sonnet-4-6
...
6. **Keep subagent prompts small.** Subagents average 41k context, p90 88k.
...
7. **Fan out wide, verify narrow.** Within the 6-agent budget above, spend it
   on distinct lenses, not redundant refuters. Verification cost is
   `findings × voters` and explodes; 2-3 lenses is enough.
8. **Never switch the main-loop model to save money.** A model switch
